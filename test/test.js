...
contract('jobscontract', accounts => {
    ...
    describe('payable fallback', () => {
        ...
        it('increases pay amount', async () => {
            ...
            const payAmountBefore = web3.utils.fromWei(payAmountWeiBefore.toString(), 'ether')
            ...
            const payAmountAfter = web3.utils.fromWei(payAmountWeiAfter.toString(), 'ether')

            // expect(payAmountAfter.toString() - payAmountBefore.toString()).to.equal("5")
    ...
    describe('sendPayAmountToTasker action', () => {
        describe('msg.sender validation', () => {
            it('can be triggered by deployer', done => {
                (async () => {
                    ...
                    deployedContract.sendPayAmountToTasker({
                            from: deployer
                        })
                        .then(() => done())
                        .catch(() => done('e'))
        ...
        it('nullifies pay amount', async () => {
            ...
            await deployedContract.sendPayAmountToTasker({
                from: deployer
            })
            ...
            expect(parseInt(payAmountAfter)).to.equal(0)
        })
    })
})